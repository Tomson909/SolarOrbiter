...
def load_nom(
    ...
    time = {}
    prods = []
    # nice! as general as possible
    for pt in products:
        for h in heads:
            for pix in pixels:
                prods.append("STEP_%s_%i_%.2i" % (pt, h, pix))
    # each directoryentrie gets data and all this
    for p in prods:
        dat[p] = []
        time[p] = []
        ...
        for f in flist:
            ...
            if p in f and tdate >= period[0] and tdate < period[1]:
                ...
                for s in fin:
                    ...
                    if "_A_" in p:
                        time[p].append(
                            dt.datetime.fromisoformat(k[0]) - dt.timedelta(seconds=60)
                        )
                    else:
                        time[p].append(
                            dt.datetime.fromisoformat(k[0]) - dt.timedelta(seconds=1)
                        )
...
def combine_data(time = [], dat = [], pixels=range(15), heads=(0, 1)):
    """
    ...
    for p in pixels:
        for h in heads:
            ...
            for i in range(32):
                H, x = np.histogram(
                    time["STEP_M_" + k], bins=tb, weights=dat["STEP_M_" + k][:, i]
                )
                td[:, 8 + i] = H
            dat["STEP_C_" + k] = td
            time["STEP_C_" + k] = tb[:-1]
