...
class CORRECT_PIXEL_AGING:
    def __init__(self,reference_pixel = 1,period=(dt.datetime(2022, 11, 18), dt.datetime(2022, 11, 19)),pixels=range(15),heads = (0,1)) -> None:
        ...
        self.average_sunwind_velocity_for_vel_vec()
        self.average_magnetic_field_for_vel_vec()
        self.correct_velocity()
        self.calc_pw()
        self.plot_pitchangle()
    ...
    def correct_velocity(self):
        ...
        for i in self.pixels:
            self.dat_STEP.velocity_vector[f'{i}_x'] -= self.dat_SWA.V_SRF['Vx']
            self.dat_STEP.velocity_vector[f'{i}_y'] -= self.dat_SWA.V_SRF['Vy']
            self.dat_STEP.velocity_vector[f'{i}_z'] -= self.dat_SWA.V_SRF['Vz']

    def plot_velocity_vector(self):
        # plot the velocity vector with seaborn
        # plot erstmal nicht nötig, da die korrektur zu passen scheint
        pass
    ...
    def test_angle_between_vectors(self):
        ...
        b = np.array([0,1,0])
        # convert to degrees
        print('angle between a and b:',np.degrees(self.angle_between_vectors(a,b)))
    ...
    def calc_pw(self):
        ...
        for i in self.pixels:
            # calculate the angle between the velocity vector and the magnetic field
            v_vel = np.array([self.dat_STEP.velocity_vector[f'{i}_x'],self.dat_STEP.velocity_vector[f'{i}_y'],self.dat_STEP.velocity_vector[f'{i}_z']])
            v_mag = np.array([self.dat_MAG.B_SRF['Bx'],self.dat_MAG.B_SRF['By'],self.dat_MAG.B_SRF['Bz']])
            self.pitchangles[f'{i}_data'] = np.degrees(self.angle_between_vectors(v_vel[:,:-1] ,v_mag))
            self.pitchangles[f'{i}_time'] = self.dat_STEP.velocity_vector[f'{i}_time'][:-1]
...
class STEP(CORRECT_PIXEL_AGING):
    ...
    def get_velocity(self,energies = None):
        # 1000 beacause energies are in keV
        return np.sqrt(2*energies*e/m_e/1000)
